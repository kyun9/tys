...
<body>
	...
	<!-- 키워드 검색 폼 -->
	<form name="form1" method="get" action="/tys/noticeBoard/selectedList" class="form-inline my-2 my-lg-0">
	...
	<table border="1">
		...
		<c:forEach var="row" items="${list }">
			<tr>
				<td>
					<c:out value="${row.n_num}"></c:out>
				</td>
				<td><a href="/tys/noticeBoard/noticeBoardDetail?n_num=${row.n_num}">
						<c:out value="${row.n_title }"></c:out>
					</a></td>
				<td>
					<c:out value="${row.n_userid }"></c:out>
				</td>
				<td>
					<c:out value="${row.n_date }"></c:out>
				</td>
				<td>
					<c:out value="${row.n_cnt }"></c:out>
				</td>
			</tr>
		</c:forEach>
